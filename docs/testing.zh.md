...
- **真实 API e2e**（`pnpm run test:e2e`）：本地显式运行的带密钥测试调用真实提供方 API，包括 DeepSeek 模型以及各提供方特有的冒烟测试；这些测试各自由自己的密钥控制（`EXA_API_KEY`、`PERPLEXITY_API_KEY` 等），缺少密钥时套件会自动跳过，仓库 CI 则保持 keyless（[CI 决策](../.agents/notes/implemented/simplification/2026-08-15-remove-real-api-e2e-workflow.md)）。
