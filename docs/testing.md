...
- **Real-API e2e** (`pnpm run test:e2e`): local, opt-in tests against live provider APIs — the DeepSeek model plus provider-specific smokes that gate on their own keys (`EXA_API_KEY`, `PERPLEXITY_API_KEY`, …); each suite self-skips without its key, and repository CI remains keyless ([CI decision](../.agents/notes/implemented/simplification/2026-08-15-remove-real-api-e2e-workflow.md)).
