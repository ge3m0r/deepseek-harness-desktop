...
`persistencePath` 为 Plugin 身份和不可变 Package 元数据与源码启用带版本的 JSON 注册表。`define` 会先通过独占 writer lock 与同目录原子 rename 提交完整的仅属主可访问文件，再发布新定义；`undefine` 先停止 Run、提交删除，再移除内存记录。启动过程会在发布服务前验证整个文件和每个已存储代码半。格式损坏、版本不受支持、源码无效、身份重复或遗留 writer lock 都会让激活明确失败，不会加载部分注册表。

Run、待处理请求、版本指针、诊断和 Client 审批授权仍只存在于进程内。恢复的 Plugin 处于停止状态，因此 Host 重启本身绝不会执行代码；后续 run 会重新经过正常审批与激活路径。省略 `persistencePath` 时保留临时和 headless 组合使用的进程内存行为。
...
| `persistencePath` | 省略 | 保存定义的 JSON 文件；Run 与审批仍只存在于进程内 |

一次 run 请求等的是人，所以这趟往返本身没有任何截止期限。
...
没有直接可见的内容：本包不注册任何工具，也不注入提示词。它的拒绝经调用它的 `cordis_*` 工具结果到达模型——无法解析的半会指出出错的那一行，缺失的定义会说明它已删除、属于另一个 Session，或当前部署没有持久化它；`rejected` 或 `cancelled` 的 run 报告的是有人拒绝或该轮次已结束而非出了故障，浏览器半装载失败则带上作答页面自己的错误文本。
...
- **遗留 writer lock 需要人工恢复。** 持久化 writer 不会删除已有 lock，因为文件年龄无法证明另一个 Host 已经停止；已提交的定义仍可读取，但 define 与 undefine 会失败，直到操作者确认没有 writer 存活后删除该 lock。
...
- **`zod` 同时验证持久注册表与生成的 TypeRT 接口。** 启动时的源码 import 会拒绝损坏的存储记录，`./typert` 与 `./remote` 则保留其未打包的运行时 import。
