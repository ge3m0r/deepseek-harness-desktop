...
动态包的 Run 只存在于共享 DSH 进程内存中。它可跨后续轮次保持活跃，也可能影响同一进程中的其他会话，但会在 `cordis_stop`、工具集卸载或 DSH 重启后消失。runner 配置 `persistencePath` 后会在重启时恢复 Plugin 身份、不可变 Package 源码与定义顺序；恢复的 Plugin 处于停止状态，也不保留 Client 审批授权。Web bundle 会在 Harness home 下启用该注册表。动态定义仍不会创建普通插件包、安装依赖、修改 `cordis.yml` 或仓库源码，也不能自动转成正式插件。需要把结果作为本地、项目或仓库插件交付时，应使用常规开发流程。每个动词都以会话为界：一个包只在定义它的会话里可见、可控，`cordis_undefine` 会删除其持久定义。
