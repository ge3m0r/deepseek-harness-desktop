...
Dynamic package Runs live only in the shared DSH process memory. They remain active across later turns and may affect other sessions in that process, but disappear after `cordis_stop`, toolset unload, or DSH restart. A runner configured with `persistencePath` restores Plugin identities, immutable Package source, and definition order after restart; restored Plugins are stopped and retain no Client approval grants. The Web bundle enables this registry under the Harness home. Dynamic definitions still create no normal Plugin package, install no dependency, change no `cordis.yml` or repository source, and cannot be promoted automatically. Use the regular development workflow when the result must ship as a local, project, or repository Plugin. Every verb is session-scoped: a package is visible and controllable only in the session that defined it, and `cordis_undefine` removes its durable definition.
